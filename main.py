...
def main():
    ...
    draw.text((400, 50), tomorrow_day_name, font=normal)
    tomorrow_weather_icon = tomorrow["icon"]
    weather_icon = Image.open(f"{image_dir}/rain.png")
    Himage.paste(weather_icon, (400, 120))
    # Temperature
    ...
    draw.text((420, 210), f"{rounded_tomorrow_temperature}°", font=normal)

    # Overmorrow's weather
    overmorrow_day_name = datetime.fromtimestamp(overmorrow["time"]).strftime("%a")
    draw.text((600, 50), overmorrow_day_name, font=normal)
    overmorrow_weather_icon = overmorrow["icon"]
    weather_icon = Image.open(f"{image_dir}/rain.png")
    Himage.paste(weather_icon, (600, 120))
    # Temperature
    ...
    draw.text((620, 210), f"{rounded_overmorrow_temperature}°", font=normal)
